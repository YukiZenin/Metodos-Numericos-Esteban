...
def runge_kutta_4(f, x0, y0, h, x_end):
    """Resuelve la EDO usando el método de Runge-Kutta de 4to Orden (RK4)."""
    ...
    # Bucle iterativo de RK4
    for i in range(n_steps):
        xi = x_vals[i]
        yi = y_vals[i]
        
        # Calcular los 4 coeficientes de Runge-Kutta
        k1 = f(xi, yi)
        k2 = f(xi + h/2, yi + (h/2) * k1)
        k3 = f(xi + h/2, yi + (h/2) * k2)
        k4 = f(xi + h, yi + h * k3)
        
        # Aplicar el promedio ponderado a la fórmula del siguiente paso
...
def f(x, y):
    return x - y  # <- ¡Igual que en Euler, solo necesitas la función original!
...
# =====================================================================
print("=" * 50)
print("     MÉTODO DE RUNGE-KUTTA 4to ORDEN (INTERACTIVO)     ")
print("=" * 50)
...
try:
    ...
    if paso_h <= 0:
        ...
        # Calcular el método
        x_res, y_res = runge_kutta_4(f, x_inicial, y_inicial, paso_h, x_final)
